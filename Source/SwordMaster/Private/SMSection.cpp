...
ASMSection::ASMSection()
{
	...
	FString AssetPath = TEXT("/Game/MyAssets/StaticMesh/SM_SQUARE.SM_SQUARE");
	...
	{
		SMLOG(Error, TEXT("Failed to load staticmesh asset. : %s"), *AssetPath);
	}
	...
	FString GateAssetPath = TEXT("/Game/MyAssets/StaticMesh/SM_GATE.SM_GATE");
	static ConstructorHelpers::FObjectFinder<UStaticMesh> SM_GATE(*GateAssetPath);
	if (!SM_GATE.Succeeded())
	{
		SMLOG(Error, TEXT("Failed to load staticmesh asset. : %s"), *GateAssetPath);
	}	

	static FName GateSockets[] = { {TEXT("+XGate")}, {TEXT("-XGate")}, {TEXT("+YGate")}, {TEXT("-YGate")} };
	for (FName GateSocket : GateSockets)
	{
		SMCHECK(Mesh->DoesSocketExist(GateSocket));
...
{
	SMCHECK(OverlappedComponent->ComponentTags.Num() == 1);
	...
	{
		SMLOG(Warning, TEXT("New section area is not empty"));
	}
}
